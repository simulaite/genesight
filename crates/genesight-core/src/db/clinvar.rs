...
use super::DbError;
use crate::models::annotation::ClinVarAnnotation;
...
pub fn query_batch(
    ...
    let mut stmt = conn.prepare(
        "SELECT c.rsid, c.clinical_significance, c.review_status, c.conditions, c.gene_symbol \
         FROM clinvar c \
    ...
    for row in rows {
        let (rsid, significance, review_stars, conditions_json, gene_symbol) = row?;
